...
class FlatMapIterator<S, T> implements SequenceIterator<T> {
    ...
    constructor(
        private readonly transform: (item: S) => ISequence<T>,
...
export class FlatMap {
     ...
     *
     * @param {(value: S) => ISequence<T>} transform
     * @returns {ISequence<T>}
     */
    flatMap<S, T>(this: ISequence<S>, transform: (value: S) => ISequence<T>): ISequence<T> {
        return of(new FlatMapIterator(transform, this.iterator));
    }
